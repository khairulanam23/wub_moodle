...
class delete_images_task extends scheduled_task {
    ...
    public function execute() {
        try {
            $service = new \quizaccess_proctoring\service\proctoring_service();
            $deletedCount = $service->process_pending_deletions(50);
            if ($deletedCount > 0) {
                mtrace("Deleted {$deletedCount} records from quizaccess_proctoring_logs and associated files.");
            } else {
