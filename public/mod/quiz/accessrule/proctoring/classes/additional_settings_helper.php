...
class additional_settings_helper {
    ...
    /** @var log_repository Proctoring log repository. */
    ...
    public function searchbycourseid($courseid) {
        return $this->repository->get_logs_by_course((int)$courseid);
    }
     ...
     * @return \moodle_recordset A recordset containing the matching logs.
     */
    public function searchbyquizid($quizid) {
        return $this->repository->get_logs_by_quiz((int)$quizid);
    }
     ...
     *
     * @return \moodle_recordset An recordset of objects containing the proctoring log data,
    ...
    public function deletelogs($deleteidstring) {
        $deleteids = array_filter(array_map('intval', explode(",", $deleteidstring)));
        if (!empty($deleteids)) {
            return $this->repository->delete_logs_and_files($deleteids);
        }
        return 0;
    }
