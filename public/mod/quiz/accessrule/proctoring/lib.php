...
function quizaccess_proctoring_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = []) {
    ...
    // Reject unauthenticated requests and guest users immediately.
    ...
    // Strict contextual authorization enforcement.
    $allowed = false;

    if (is_siteadmin()) {
        $allowed = true;
    } else if ($filearea === 'picture' || $filearea === 'face_image') {
        if ($context->contextlevel == CONTEXT_MODULE || $context->contextlevel == CONTEXT_COURSE) {
            // Module (Quiz) or Course context.
            if (has_capability('quizaccess/proctoring:viewreport', $context)) {
                $allowed = true;
            } else {
                // Students can only access their own webcam pictures / face crops.
                $log = $DB->get_record('quizaccess_proctoring_logs', ['id' => $itemid], 'id, userid, quizid, courseid');
                if ($log && (int)$log->userid === (int)$USER->id) {
                    $allowed = true;
                } else if ($filearea === 'face_image') {
                    $face = $DB->get_record('quizaccess_proctoring_face_images', ['id' => $itemid], 'id, parent_type, parentid');
                    if ($face) {
                        $parentlog = $DB->get_record('quizaccess_proctoring_logs', ['id' => $face->parentid], 'id, userid');
                        if ($parentlog && (int)$parentlog->userid === (int)$USER->id) {
                            $allowed = true;
                        }
                    }
                }
            ...
            // System context face images / user baseline photos.
            if ((int)$USER->id === $itemid) {
                $allowed = true;
            } else if (has_capability('moodle/site:config', $context) || has_capability('quizaccess/proctoring:viewreport', $context)) {
                $allowed = true;
            }
        }
    } else if ($filearea === 'user_photo') {
        // System context reference photo.
        if ((int)$USER->id === $itemid) {
            $allowed = true;
        } else if (has_capability('moodle/user:viewdetails', $context) || has_capability('quizaccess/proctoring:viewreport', $context)) {
            $allowed = true;
        }
    }
