...
function xmldb_quizaccess_proctoring_upgrade($oldversion) {
    ...
    if ($oldversion < 2026082201) {
        ...
        // 1. Composite index on (courseid, quizid)
        $index1 = new xmldb_index('course_quiz_idx', XMLDB_INDEX_NOTUNIQUE, ['courseid', 'quizid']);
        if (!$dbman->index_exists($table, $index1)) {
            $dbman->add_index($table, $index1);
        }

        // 2. Index on userid
        $index2 = new xmldb_index('userid_idx', XMLDB_INDEX_NOTUNIQUE, ['userid']);
        if (!$dbman->index_exists($table, $index2)) {
            $dbman->add_index($table, $index2);
        }

        // 3. Index on deletionprogress
        $index3 = new xmldb_index('deletionprogress_idx', XMLDB_INDEX_NOTUNIQUE, ['deletionprogress']);
        if (!$dbman->index_exists($table, $index3)) {
            $dbman->add_index($table, $index3);
        }

        upgrade_plugin_savepoint(true, 2026082201, 'quizaccess', 'proctoring');
    }
